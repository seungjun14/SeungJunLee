...
# Note2. __를 붙여서 내부전용으로 외부에서 부르는 함수와 구분합니다.
def __getSentiment(userContent):
    ...
    if(response.status_code == 200):
        ...
        raise Exception(str(response.status_code)) # Note3. Error인 경우 아무것도 명시하지 않음. return을 해주거나 exception을 일으켜주어서 마감을 해줄 필요가 있음 #1. Error인 경우 아무것도 명시하지 않음. return을 해주거나 exception을 일으켜주어서 마감을 해줄 필요가 있음. 그래야지 이 함수를 호출하는 곳에서 거기에 맞는 처리를 할 수가 있음

# Note4. 공통으로 사용하는 기능을 함수로 분리하여 재사용합니다.
def __displayOutSentences(message, sentences):
    print (message)
    for sentence in sentences: print (sentence)
    print ('\n')

# Note5. 함수는 하나의 목적을 갖는게 좋습니다.
...
def getSituationFeedback():
    ...
    print ('\n')
    while True:
        # Note6. 앞서 Exception raise 하는 것을 try except로 처리해줍니다.
        try:
        ...
        # Note8. feedback message 길이로 판단하는 것보다는 정확한 data로 판단하는게 정확합니다.
        # Note9. 아래와 같이 생각의 흐름을 그대로 기술하듯이 코딩하는게 좋습니다. 
        if neutralScore > 30:
